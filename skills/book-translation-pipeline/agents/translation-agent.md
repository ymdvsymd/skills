...
- Markdown 構造: 見出しレベル・リスト・表・コードブロックを完全保持。**見出しは原文と同じ `#` の数** (`##`→`###` 等にずらさない)。**番号付きリスト (`1.` `2.`) は番号付きのまま訳出** (平文段落に潰さない)
- サイドバー/コラム: 原文 `> **Sidebar: <title>**` は **`> **コラム: <title>**`** に統一。本文の全行を `>` で囲み、裸段落・`##### title`・別行 `コラム` 等の揺れた書式を使わない。Recap 系の小見出し (`> **小見出し**`) は原文と同数を保持。空の引用区切りは bare `>` (末尾スペースなし)
- markdownlint: 行末スペースを付けない (ハードブレイクが要る箇所のみスペース2個 / MD009)。blockquote ブロック間に空行を挟まない (MD028)
...
` ```language ... ``` ` フェンス済みコードブロックは**原文を文字通りコピー**する:

- フェンス記号 ` ``` ` (3 連続バッククォート) と言語識別子 (`python`, `sql`, `bash` 等) を保持
- **フェンス済みブロックを `` `def` `allocate` `(` `line` `` のような単一バッククォート inline code の連なりに分解してはならない** (これは過去に発生した実際のバグパターン)
- コード行内のコメント (`# ...`) も英語原文のまま (訳さない)
- インデントもそのまま保持

完了前のセルフチェック:
- `node scripts/check-code-fragments.mjs docs/ja/__FILE__` が exit 0 で通ること
- `grep -c '^```' docs/en/__FILE__` と `grep -c '^```' docs/ja/__FILE__` の件数が一致すること
...
5. セルフチェック:
   ...
   - 書名/著者名は原文維持
   - コードブロック断片化なし (`node scripts/check-code-fragments.mjs docs/ja/__FILE__` が exit 0)
   - en/ja の ``` フェンス件数一致 (`grep -c '^```' docs/{en,ja}/__FILE__` が同値)
   - 構造パリティ (`node scripts/check-structure-parity.mjs __FILE__` が hard mismatch ゼロ。サイドバー個数・フェンス数・見出しレベルが en/ja で一致)
